# -*- coding: utf-8 -*-
"""
Tải ảnh siêu tốc — Ứng dụng GUI (PySide6)

Bổ sung: Auto-update online
- Nút "Kiểm tra cập nhật" (menu Trợ giúp) để tải & tự thay thế .exe
- Dùng manifest JSON public (GitHub/Gist/Server) để thông báo version & URL tải

Các tính năng sẵn có:
- Logo/app icon, Dark mode bật/tắt
- Đa luồng tăng tốc tải ảnh (1–32 luồng)
- Tải 1 URL hoặc danh sách URL (.txt)
- Tự nhận dạng đuôi theo Content-Type, giữ tên gốc
- Lọc định dạng, lọc ảnh nhỏ (min-bytes)
- Khử trùng theo nội dung (SHA1)
- Chọn bản lớn nhất từ biến thể (-WxH, /236x/, srcset, query ?w=...) 
- Progress bar + log, Hủy, Mở thư mục

Đóng gói Windows (.exe):
  pip install pyinstaller
  pyinstaller --onefile --windowed --collect-all PySide6 --name "TaiAnhSieuToc" image_downloader_app.py
  (tùy chọn) --icon your_icon.ico
"""
...
# Tự cài các lib cốt lõi khi chạy từ source
...
PIN_DIR_RE = re.compile(r"/(\d{2,5})x/")  # /236x/, /736x/
DATA_URL_RE = re.compile(r"^data:([^;,]+)?((?:;[^,]+)*)?,(.*)$", re.I)

# Hints từ srcset/query
HINT_SIZES = {}

# ====== App Icon (embedded base64 PNG 64x64) ======
_APP_ICON_B64 = (
...
def extract_query_size(full_url: str):
    ...
    def first_int(key):
        if key in q and q[key]:
            try:
                return int(str(q[key][0]).split(",")[0].split("x")[0])
            except:
                return None
        return None
    def pair_from(key):
...
def pick_largest_variants(session, urls: list[str]) -> list[str]:
    ...
    for u in urls:
        if u.startswith("data:"):
            key = f"DATA::{hash(u)}"
            buckets.setdefault(key, []).append((u, None, None))
            continue
        parsed = urlparse(u)
    ...
    for key, cands in buckets.items():
        ...
        if with_sizes:
            best = max(with_sizes, key=lambda c: (c[2][0] * c[2][1]))
...
def extract_image_urls(html, base_url):
        ...
        if srcset:
            for part in srcset.split(","):
                ...
                if len(tokens) >= 2:
                    ...
                    if d.endswith("w"):
                        try:
                            w = int(d[:-1]); HINT_SIZES[full] = (w, w)
                        except: pass
                    elif d.endswith("x"):
                        try:
                            mul = float(d[:-1]); w = int(mul * 1000); HINT_SIZES[full] = (w, w)
                        except: pass
    ...
    for tag in soup.find_all(style=True):
        for m in re.finditer(r"url\((['\"]?)(.+?)\1\)", tag["style"]):
            u = m.group(2)
            if not u: continue
            full = u if u.startswith("data:") else urljoin(base_url, u)
            urls.add(full)
    # <a href="...">
...
# ====== Save/Download ======
def sanitize_allow_list(txt: str) -> set:
    return set([e.strip().lower() for e in txt.split(",") if e.strip()])
...
def save_bytes(raw: bytes, out_dir: str, filename: str, min_bytes: int, allow_exts: set, seen_hashes: set, lock: Lock | None = None):
    if min_bytes and len(raw) < min_bytes:
        return False, f"Bỏ qua (nhỏ hơn {min_bytes}B): {filename}"
    ext = os.path.splitext(filename)[1].lower()
    if allow_exts and ext and ext[1:] not in allow_exts:
        return False, f"Bỏ qua (không nằm trong allow): {filename}"
    h = hashlib.sha1(raw).hexdigest()
    if lock:
        with lock:
            if h in seen_hashes:
                return False, f"Bỏ qua (trùng nội dung): {filename}"
            seen_hashes.add(h)
    else:
        if h in seen_hashes:
            return False, f"Bỏ qua (trùng nội dung): {filename}"
...
def download_http_image(session, img_url, out_dir, referer, min_bytes, allow_exts, seen_hashes, lock: Lock | None = None):
    ...
    try:
        r = session.get(img_url, stream=True, timeout=20, headers=headers, allow_redirects=True)
    except requests.RequestException as e:
        return False, f"Lỗi tải {img_url} -> {e}"
...
"""
    ...
    with open(up_path, 'w', encoding='utf-8', errors='ignore') as f:
        f.write(bat)
    return up_path
...
class DownloaderWorker(QtCore.QThread):
    ...
    def __init__(self, pages: list[str], out_dir: str, allow_exts: set, min_bytes: int,
                 accept_data: bool, auto_referer: bool, explicit_referer: str,
                 max_workers: int):
        super().__init__()
        self.pages = pages
    ...
    def _derive_referer(self, page_url: str) -> str:
        if self.explicit_referer:
            return self.explicit_referer
        if not self.auto_referer:
            return ""
        try:
            p = urlparse(page_url)
    ...
    def _download_one(self, img_url: str, referer: str) -> bool:
        ...
        if img_url.startswith("data:"):
            ...
            try:
                ...
                success, msg = save_bytes(raw, self.out_dir, fname, self.min_bytes,
                                          self.allow_exts, self.seen_hashes, self.hash_lock)
                self.log_msg.emit(msg)
                return success
            except Exception as e:
                self.log_msg.emit(f"Lỗi data URL -> {e}")
                return False
        # http/https
        session = build_session()
        success, msg = download_http_image(session, img_url, self.out_dir, referer or img_url,
                                           self.min_bytes, self.allow_exts, self.seen_hashes, self.hash_lock)
        self.log_msg.emit(msg)
        return success
    ...
    def run(self):
        ...
        # Thu thập URL ảnh từ tất cả trang
        collected = []
        for page_url in self.pages:
            ...
            try:
                ...
                self.log_msg.emit(f"Lỗi tải trang {page_url}: {e}")
                continue
            urls = extract_image_urls(page.text, page_url)
            if not urls:
                self.log_msg.emit(f"Không tìm thấy ảnh ở: {page_url}")
                continue
        ...
        # Gom biến thể và lọc trùng URL
        final_urls = pick_largest_variants(session, list(dict.fromkeys(collected)))
        total = len(final_urls)
        self.log_msg.emit(f"Sau khi gom biến thể, còn {total} URL cần tải.")
        if total == 0:
            self.finished.emit(0, 0)
            return
...
class MainWindow(QMainWindow):
    def __init__(self, app: QApplication):
        super().__init__()
        self.app = app
        self.setWindowTitle("Tải ảnh siêu tốc")
        ...
        help_menu = QMenu("Trợ giúp", self)
        menubar.addMenu(help_menu)
        act_check = QAction("Kiểm tra cập nhật", self)
        ...
        self.out_edit = QLineEdit(os.path.join(os.getcwd(), "images"))
        self.btn_out = QPushButton("Chọn thư mục…")
        ...
        self.btn_open = QPushButton("Mở thư mục lưu")
        ...
        grid.addWidget(QLabel("Số luồng tải:"), 6, 0); grid.addWidget(self.workers_spin, 6, 1)
        grid.addWidget(self.dark_cb, 6, 2)
    ...
    def check_updates(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Kiểm tra cập nhật", f"Không tải được manifest: {e}")
            return
        latest = str(manifest.get("version", "0.0.0"))
        cur = parse_version_tuple(__version__)
        lat = parse_version_tuple(latest)
        if lat <= cur:
            QMessageBox.information(self, "Kiểm tra cập nhật", f"Bạn đang dùng bản mới nhất ({__version__}).")
            return
        # lấy info windows
        win = manifest.get("windows", {})
        url = win.get("url")
        sha = (win.get("sha256") or "").lower()
        if not url:
            QMessageBox.warning(self, "Kiểm tra cập nhật", "Manifest thiếu URL tải cho Windows.")
            return
        # hỏi tải
        if QMessageBox.question(self, "Có bản mới", f"Phát hiện phiên bản mới {latest}.\nBạn có muốn tải và cài đặt ngay không?") != QMessageBox.Yes:
            return
        ...
        new_path = os.path.join(tmp_dir, f"TaiAnhSieuToc_{latest}.exe")
        ...
        if sha:
            ...
            if calc != sha:
                ...
                return
        # tạo updater và thoát để thay thế
        ...
        try:
            ...
            return
        # thoát app để updater thay thế
        QApplication.instance().quit()

    # ===== UI actions =====
    def toggle_dark(self, checked: bool):
        apply_dark_mode(self.app, checked)
    ...
    def start_download(self):
        ...
        if not pages:
            ...
            if url:
                pages = [url]
        if not pages:
            self.log.append("❗ Vui lòng nhập URL hoặc chọn file .txt danh sách URL.")
            return

        out_dir = self.out_edit.text().strip() or os.path.join(os.getcwd(), "images")
        allow_exts = sanitize_allow_list(self.allow_edit.text())
        min_bytes = int(self.min_spin.value())
        accept_data = not self.cb_no_data.isChecked()
        auto_ref = self.cb_auto_ref.isChecked()
        ...
        self.worker.log_msg.connect(self.log.append)
...
if __name__ == "__main__":
    from PySide6.QtWidgets import QMainWindow  # noqa: E402 (để type hints)
